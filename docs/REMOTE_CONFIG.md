...
The flag applies globally, with no storefront restriction. Failed requests and
malformed responses hide the link. JSON booleans must be lowercase `true` or `false`.
There is no persistent enabled cache. An already open page refreshes
...
Disclose the support link and remote flag in App Review
...
Global visibility does not establish eligibility under every storefront's review rules.
